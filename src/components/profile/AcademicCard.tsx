    ...
    >
      ...
      <div className="relative flex items-start justify-between gap-3">
        <div className="min-w-0">
          <div className="text-[10px] uppercase tracking-normal text-muted-foreground">Academic</div>
          <h2 className="text-xl md:text-xl font-semibold leading-tight truncate">{data.displayName}</h2>
        ...
        <div className="flex-shrink-0 rounded-xl px-2.5 py-1.5 text-center" style={{ background: `${rank.color}22`, border: `1px solid ${rank.color}55` }}>
          ...
          <div className="text-[10px] font-semibold mt-0.5 uppercase tracking-normal" style={{ color: rank.color }}>{rank.name}</div>
        </div>
      </div>
      <div className="relative mt-4 grid grid-cols-2 gap-2">
        <div className="rounded-xl border border-border bg-card/60 py-2 text-center">
          <div className="text-base md:text-lg font-semibold tabular-nums">{data.gpa != null ? data.gpa.toFixed(2) : "—"}</div>
          <div className="text-[9px] uppercase tracking-normalst text-muted-foreground">GPA {data.gpaWeighted ? "(Weighted)" : ""}</div>
        </div>
        <div className="rounded-xl border border-border bg-card/60 py-2 text-center">
          <div className="text-base md:text-lg font-semibold tabular-nums">{data.classes?.length ?? 0}</div>
          <div className="text-[9px] uppercase tracking-normalst text-muted-foreground">Classes</div>
        </div>
      </div>
      {data.classes && data.classes.length > 0 && (
        <div className="relative mt-4">
          <div className="text-[10px] uppercase tracking-normalst text-muted-foreground mb-1.5">Classes</div>
