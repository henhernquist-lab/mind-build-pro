...
do $$
begin
  if not exists (select 1 from pg_policies where policyname = 'own sessions') then
    create policy "own sessions" on workout_sessions for all using (auth.uid() = user_id);
  end if;
  if not exists (select 1 from pg_policies where policyname = 'own exercises') then
    create policy "own exercises" on session_exercises for all using (auth.uid() = user_id);
  end if;
  if not exists (select 1 from pg_policies where policyname = 'own sets') then
    create policy "own sets" on session_sets for all using (auth.uid() = user_id);
  end if;
  if not exists (select 1 from pg_policies where policyname = 'own prs') then
